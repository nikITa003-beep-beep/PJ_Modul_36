    ...
    m_userName(userName)
{
    ...
    connect(m_updateTimer, &QTimer::timeout, this, &MainWindow::updateChats);
...
{
    if (m_dbPtr->isUserBanned(m_userName.toStdString())) {
        QMessageBox::warning(this, tr("Warning"), tr("You are banned and cannot send messages"));
        return;
    }
...
void MainWindow::updateChats()
{
    if (m_dbPtr->isUserBanned(m_userName.toStdString())) {
        ...
        ui->privateMessageSendButton->setEnabled(false);
        ui->messageLineEdit->setEnabled(false);
        ui->statusbar->showMessage(tr("You are banned from the chat"));
    } else {
        ui->sendMessageButton->setEnabled(true);
        ui->privateMessageSendButton->setEnabled(true);
    ...
    for(const auto &msg : privateMessages)
    {
        ...
        else if(m_userName == QString::fromStdString(msg.getSender()))
        {
            prefix = tr("message to") +
                    QString(" <%1>: ").arg(QString::fromStdString(m_dbPtr->getUserName(msg.getDest())));
        }
        else
        {
